...
Developing the automatic adaptation of the output to the limits of the respective application window is challenging. Only a few VNC and RDP servers make exporting the output of only individual windows or processes possible~\cite{OJCC_2023v8i1n01_Baun}. For Windows operating systems, the proprietary built-in RDP server and the free software TightVNC offer the export of full desktops and single windows. For Linux operating system deployments, the free software projects xrdp and x11vnc enable all desired features (see Table~\ref{tab:DaaS_Services_Overview}). 
...
\begin{table}
	...
	\begin{tabular}{|l|c|c|c|c|}
		\hline
		\textbf{Service}    &  \textbf{Protocol}  & \textbf{Export of}           & \textbf{Operating} & \textbf{Software}  \\
		\textbf{implementation}  &                & \textbf{single Window only}  & \textbf{System}    & \textbf{License}   \\
		\hline
